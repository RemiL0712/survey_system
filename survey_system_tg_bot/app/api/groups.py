...
router = APIRouter(prefix="/groups", tags=["groups"], dependencies=[Depends(require_api_access)])
...
@router.get("/{group_id}/memberships/{user_id}")
async def group_membership(group_id: int, user_id: int, session: AsyncSession = Depends(get_session)):
    ...
    return {"group_id": group_id, "user_id": user_id, "is_member": result.scalar_one_or_none() is not None}
...
async def remove_member(group_id: int, user_id: int, session: AsyncSession = Depends(get_session)):
    ...
    # РґС–СЃС‚Р°РЅРµРјРѕ РґР°РЅС– РґР»СЏ РїРѕРІС–РґРѕРјР»РµРЅРЅСЏ
    ...
    # РІРёРґР°Р»СЏС”РјРѕ С‡Р»РµРЅСЃС‚РІРѕ
...
async def delete_group(group_id: int, session: AsyncSession = Depends(get_session)):
    ...
    # РїРµСЂРµРІС–СЂРєР° С–СЃРЅСѓРІР°РЅРЅСЏ РіСЂСѓРїРё
    ...
    # С‡РёСЃС‚РёРјРѕ Р·Р°Р»РµР¶РЅРѕСЃС‚С– (С‰РѕР± РЅРµ Р±СѓР»Рѕ FK РїСЂРѕР±Р»РµРј)
...
async def list_group_surveys(group_id: int, user_id: int, session: AsyncSession = Depends(get_session)):
    ...
    # вњ… РґРѕСЃС‚СѓРї С‚С–Р»СЊРєРё С‡Р»РµРЅР°Рј РіСЂСѓРїРё Р°Р±Рѕ creator
    ...
    if (
        ...
        and group.created_by != user_id
        and not await is_bot_admin(session, group.bot_id, user_id)
    ):
