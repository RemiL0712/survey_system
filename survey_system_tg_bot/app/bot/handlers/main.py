...
def parse_admin_tg_ids() -> set[int]:
    ...
    for raw_source in (ADMIN_TG_ID_RAW, ADMIN_TG_IDS_RAW):
...
def register_handlers(dp, bot: Bot, client: httpx.AsyncClient):
    ...
    async def start(m: Message):
        ...
        is_admin = is_admin_user(m.from_user.id)
    ...
    async def admin_groups(cb: CallbackQuery):
        ...
        if not is_admin_user(cb.from_user.id):
        ...
        try:
            ...
            if e.response is not None and e.response.status_code == 403:
                await render_menu_from_callback(
                    cb,
                    "� ��� ���� ������� �� ��������� ���� �����.",
                    reply_markup=kb_group_actions(group_id, is_admin=is_admin, is_member=False),
                )
                return
            raise
    ...
    async def kick(cb: CallbackQuery):
        ...
        try:
            ...
            resp = await api_delete(client, f"/groups/{group_id}/members/{user_id}")



            try:

                await show_main_menu_to_chat(
                    bot,
                    int(resp["user_telegram_id"]),
                    is_admin=False,
                    text=f"ℹ️ Вас видалено з групи «{resp['group_name']}».\n\nГотово. Обери дію:",
                )
    ...
    async def approve(cb: CallbackQuery):
        ...
        try:
            ...
            if resp.get("already_processed"):
                ...
                await cb.answer(f"ℹ️ Заявка вже оброблена: {resp['status']}", show_alert=False)
            ...
            try:
                ...
                await show_main_menu_to_chat(
                    ...
                    int(resp["user_telegram_id"]),
                    is_admin=False,
                    text=f"✅ Вашу заявку до групи «{resp['group_name']}» прийнято.\n\nГотово. Обери дію:",
                )
        ...
        try:
            ...
            try:
                ...
                await show_main_menu_to_chat(
                    ...
                    int(resp["user_telegram_id"]),
                    is_admin=False,
                    text=f"❌ Вашу заявку до групи «{resp['group_name']}» відхилено.\n\nГотово. Обери дію:",
                )
    ...
    async def pending(cb: CallbackQuery):
        ...
        for it in items[:50]:
            ...
            request_message_ids = admin_req_message_id.setdefault(req_id, {})
            for admin_chat_id in ADMIN_TG_IDS:
                mid = request_message_ids.get(admin_chat_id)
                if mid:
                    try:
                ...
                request_message_ids[admin_chat_id] = msg.message_id
    ...
    async def delete_group_confirm(cb: CallbackQuery):
        ...
        try:
            ...
            await api_delete(client, f"/groups/{group_id}")
